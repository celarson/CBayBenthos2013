...
CBayspChao1<-read.csv("ezD7782405_APTC_SG1_singdoub_Lymn.csv",header=T)
names(CBayspChao1)
...
CBayspChao2<-read.csv("ezD7782400_APTC_SG1_unidup_Lymn.csv",header=T)
names(CBayspChao2)
#change dash to underscore for site names
...
#Upload clusters
CBayCluster<-read.csv("ezD9863780_CBay_Clusters.csv",header=T)
...
max(CBayPonarMatrixCh1Env$HabDepth)
...
#Create triangle plots with % in same cluster on x axis and % deep on y axis
# deep is greater than 2.6
#iterate on n=16, greatest efficiencies in sampling occur in the early portion of the species-effort curve (Hoffman et al. 2011)
#10 random draws

#do this in a simplified way first.

#Add clusters to the matrix/environmental dataset
CBayPonarMatrixCh1EnvClust<-merge(CBayPonarMatrixCh1Env,CBayCluster,By="InvEventFK")
#now have 272 observations because of one deleted

#Create "shallow" and "deep" variable
CBayPonarMatrixCh1EnvClust$Shallow<-cut(CBayPonarMatrixCh1EnvClust$HabDepth,
                                        breaks=c(0, 2.6, 35),
                                        labels=c('Shallow', 'Deep'))
##################################
#Only Run if need randomization analysis
#Code adapted from Kelsey Vitense
#############################
# Total number of sampled sites stays fixed
num_sites <- 16

# Number of clusters needs to divide 16 in order to have equal number of sites in each cluster
num_clusters <- c(1, 2, 4, 8) # , 16   Do 16 separately because can only have 8 shallow and 8 deep clusters

# Get sequence of possible number shallow sites for each number of clusters
num_shallow <- NULL
for (i in num_clusters){
  num_shallow <- c(num_shallow, seq(0,i,1))
}

# Create data frame with parameter combinations - adding 16 cluster situation tacked to the end
Par_Table <- data.frame(Num_Clusters=c(rep(num_clusters, times=num_clusters+1), 16),
                        Num_Shallow_Clusters=c(num_shallow, 8))

# Add proportion column and number of sites per cluster
Par_Table$Proportion_Shallow_Clusters <- Par_Table$Num_Shallow_Clusters / Par_Table$Num_Clusters
Par_Table$Num_Sites_Per_Cluster <- num_sites/Par_Table$Num_Clusters
Par_Table

# Expand parameter table for number replicates at each parameter combination
Replicate_Table <- Par_Table[rep(seq_len(nrow(Par_Table)), each = 100), ]
Replicate_Table$Replicate <- rep(1:100, nrow(Par_Table))
row.names(Replicate_Table) <- NULL

# Put in place holder columns for statistics you will calculate
Replicate_Table$Chao1<- NA

# Unique shallow and deep cluster IDs
shallow_cluster_ids <- unique(CBayPonarMatrixCh1EnvClust$Cluster[CBayPonarMatrixCh1EnvClust$Shallow=="Shallow"])
deep_cluster_ids <- unique(CBayPonarMatrixCh1EnvClust$Cluster[CBayPonarMatrixCh1EnvClust$Shallow=="Deep"])

# Loop to sample clusters and sites, and store replicate statistics
# Set seed for reproducibility
set.seed(37)

for (i in 1:nrow(Replicate_Table)){
  
  print(i)
  
  num_shallow_i <- Replicate_Table$Num_Shallow_Clusters[i]
  num_deep_i <- Replicate_Table$Num_Clusters[i] - Replicate_Table$Num_Shallow_Clusters[i]
  num_sites_i <- Replicate_Table$Num_Sites_Per_Cluster[i]
  
  # Sample shallow clusters
  samp_shall_clust <- sample(shallow_cluster_ids, num_shallow_i, replace=FALSE)
  # Sample deep clusters
  samp_deep_clust <- sample(deep_cluster_ids, num_deep_i, replace=FALSE)
  
  # Combine the clusters (note: would need to keep separate and modify code if end up sampling different number of sites for shallow vs. deep clusters)
  clusts_i <- c(samp_shall_clust, samp_deep_clust)
  
  # Sample sites from each cluster
  sites_i <- NULL
  for(cl in clusts_i){
    # aggregate sampled sites from each sampled cluster
    sites_i <- c(sites_i, sample(CBayPonarMatrixCh1EnvClust$InvEventFK[CBayPonarMatrixCh1EnvClust$Cluster==cl], num_sites_i, replace=FALSE))
  }
  
  # If more than 16 sites were sampled, stop and throw an error and fix the code.
  if(length(sites_i)>16) stop("More than 16 sites were sampled! Something is wrong.")
  
  # Subset dataset to sampled sites
  dat_i <- CBayPonarMatrixCh1EnvClust[CBayPonarMatrixCh1EnvClust$InvEventFK %in% sites_i, ]
  
  # *** Calculate statistics here using data from sampled sites in dat_i ***
  Replicate_Table$Chao1[i] <- chao1(dat_i[2:254],taxa.row=F)
  
}
chao1sum<-summarySE(Replicate_Table, measurevar="Chao1",  groupvars=c("Proportion_Shallow_Clusters","Num_Clusters"))

ggplot(chao1sum, aes(Num_Clusters, Proportion_Shallow_Clusters))+
  geom_tile(aes(fill = Chao1))+
  scale_fill_gradient(low = "#ffffd9", high = "#081d58", name="Chao1Richness")+
  labs(x="Number of clusters", y="Proportion shallow")+
  theme_bw()
ggplot(chao1sum, aes(x=Num_Clusters, y=Chao1,colour=Proportion_Shallow_Clusters)) +
  geom_point(size=2) +
  geom_errorbar(aes(ymin=Chao1-se, ymax=Chao1+se), width=.4) +  
  xlab("Number of Clusters")+
  ylab("Chao1 Richness")+
  labs(colour="Prop.Shallow")+
